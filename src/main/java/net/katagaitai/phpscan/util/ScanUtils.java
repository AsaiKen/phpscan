...
public class ScanUtils {
    ...
    public static void report(Interpreter ip, List<Vulnerability> vulnerabilityList) {
        ...
        for (int pageNum = 0; pageNum < Math.ceil(((double) size) / vulnSizePerPage); pageNum += 1) {
            ...
            if (subList.size() > 0) {
                ...
                try {
                    String headString = Files.toString(
                            new File(ScanUtils.class.getClassLoader().getResource("report_template.head").getFile()),
                            Constants.DEFAULT_CHARSET);
                    String tailString = Files.toString(
                            new File(ScanUtils.class.getClassLoader().getResource("report_template.tail").getFile()),
                            Constants.DEFAULT_CHARSET);
    ...
    public static String getStackTraceWithSource(List<SourcePosition> callStack) {
        ...
        for (int i = 0; i < callStack.size(); i++) {
            ...
            list.add(String.format("#%d: %s\n\t%s", callStack.size() - i, position.toString(),
                    position.getSourceLine()));
        }
        return String.join("\n", list);
    }
