    ...
    protected String make_declaration(
        ...
        if (emit.lr_values()) {
            if (!emit.locations())
                ...
                ret =
                        "\t\tLocation " + labelname + "xleft = ((java_cup.runtime.ComplexSymbolFactory.ComplexSymbol)" +
                                emit.pre("stack")
                                +
                                // TUM 20050917
                                ((offset == 0) ? ".peek()" : (".elementAt(" + emit.pre("top") + "-" + offset + ")")) +
    ...
    protected void remove_embedded_actions(
        ...
        for (int act_loc = 0; act_loc < rhs_length(); act_loc++)
            if (rhs(act_loc).is_action()) {
                ...
                new_nt = non_terminal
                        .create_new(null, lhs().the_symbol().stack_type()); // TUM 20060608 embedded actions patch
                ...
                new_prod =
                        new action_production(this, new_nt, null, 0,
                                declare_str + ((action_part) rhs(act_loc)).code_string(), (lastLocation == -1) ? -1
