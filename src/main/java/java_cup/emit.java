...
public class emit {
    ...
    protected static void emit_action_code(PrintWriter out, production start_prod)
        ...
        for (int instancecounter = 0; instancecounter <= production.number() / UPPERLIMIT; instancecounter++) {
            ...
            for (; proditeration < Math.min((instancecounter + 1) * UPPERLIMIT, production.number()); prod =
                    production.find(++proditeration)) {
                /* case label */
                out.println("          /*. . . . . . . . . . . . . . . . . . . .*/");
                    ...
                    if (lastResult != -1) {
                        result =
                                "(" + prod.lhs().the_symbol().stack_type()
                                        ...
                                        +
                                        // TUM 20050917
                                        ((lastResult == 1) ? ".peek()" : (".elementAt(" + emit.pre("top") + "-"
                                                + (lastResult - 1) + ")")) +
                                        ").value";
                    }
                }
                ...
                if (emit.lr_values()) {
                    ...
                    if (prod.rhs_length() == 0)
                        ...
                        leftstring =
                                "((java_cup.runtime.Symbol)"
                                        ...
                                        +
                                        // TUM 20050917
                                        ((loffset == 0) ? (".peek()") : (".elementAt(" + emit.pre("top") + "-"
                                                + loffset + ")")) +
                    ...
                    out.println("              " + pre("result") + " = parser.getSymbolFactory().newSymbol(" +
                            ...
                            ", " + leftstring + ((prod.rhs_length() == 0) ? ("") : (", " + rightstring)) +
                            ", RESULT);");
                } else {
                    out.println("              " + pre("result") + " = parser.getSymbolFactory().newSymbol(" +
                            "\"" + prod.lhs().the_symbol().name() + "\"," +
            ...
            for (; proditeration < Math.min((instancecounter + 1) * UPPERLIMIT, production.number()); prod =
                ...
                for (int rhsi = 0; rhsi < prod.rhs_length(); rhsi++) {
                    ...
                    if (sym.the_symbol().is_non_term())
                        ...
                        nested +=
                                ",new XMLElement.Terminal(" + label + "xleft,\"" + label + "\"," + label + "," + label
                                        + "xright)";
                }
                ...
                if (emit.lr_values()) {
                    ...
                    out.println("              " + pre("result") + " = parser.getSymbolFactory().newSymbol(" +
                            "\"" + prod.lhs().the_symbol().name() + "\"," + prod.lhs().the_symbol().index()
                            + ", RESULT);");
                }
